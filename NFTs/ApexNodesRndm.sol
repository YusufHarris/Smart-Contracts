...
// File: contracts/SquatchNodes.sol
...
contract HypeNodes is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  mapping(uint256 => uint) public nodeType;
      uint256[3] public mintPrices=[0,0,0];
  ...
  function mint(uint _type) public payable {
    ...
    uint256 cost = mintPrices[_type-1];
    ...
    returns (string memory)
  {
    ...
    return bytes(currentBaseURI).length > 0
        ? string(abi.encodePacked(currentBaseURI, nodeType[tokenId].toString(), baseExtension))
        : "";
  }
